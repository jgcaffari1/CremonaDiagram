...
public class Vertex<K extends Comparable<K>, V> {
  ...
  /**
   * created a weighted connection, or updates the weights if the connection already exists.
   * 
   ...
   * @return the edge starting at the current vertex and ending at the vertex with the given key, or
  ...
  /**
   * checks if the current vertex is connected to the vertex with the specified key
   * 
  ...
  /**
   * gets all outEdges of this vertex. Keys are indexed with the key where the edge ends.
   * 
   * @return all outEdges.
   */
  ...
  /**
   * these compare algorithms will sort items in descending order when used in a priority queue - so
   * strings will be sorted in reverse alphabetical order
   */
