...
/**
 * graph visualizer class - creates a gui for displaying and interacting with the graph data.
 * structure.
 * 
 * @author jgcaf
 */
public class Visualizer extends PApplet {
  ...
  public Visualizer() {
    // create a priority queue that will sort selected vertexes from closest to farthest from mouse.
  ...
  @Override
  public void setup() {
    // only build the gui and initialize structures upon startup, then just reset items:
    if (!reset) {
  ...
  public static void main(java.lang.String[] args) {
    PApplet.main("graphVis.Visualizer");
  }
  ...
  /**
   * Forms or Changes an undirected edge between the vertexes with the specified id numbers
   * 
  ...
  /**
   * plots the nodes and edges of the graph, with friction slowing all edges and with
   */
  ...
  /**
   * allows the user to select the two vertexes at the ends of the an edge with the mouse.
   * 
  ...
  private void drawEdge(Edge<Integer, Mover> c) {
    ...
    if (c != null) {
      ...
      if (mouseOverEdge(c)) {
        stroke(255, 0, 0);
        cp5.get(Textfield.class, "weight").setColor(color(255, 255, 0))
            .setText(Double.toString(c.getWeight()));
  ...
  /**
   * clears the text from the given text box when the user activates it if it matches the default
   * text.
   * 
  ...
  /**
   * +" q - exit" +" r - reset" +" n - add new ball" +" p - pin/freeze all items" +" d - unfreeze
   * all items" +" e - erase vertex" +" z - unselect all vertexes" +" a - add undirected edge" +" s
   * - add directed edge start-> finish" +" t - delete edge" +" y - delete directed edge start ->
   * finish" +" l - reset spanning tree/ display main graph" +" k - turn sound on" +" o - turn sound
   * off"
   */
  @Override
  ...
  private void saveMoverSequential(Mover m) {
    ...
    // if a mover that has not been clicked before is selected when two are already selected, then
  ...
  /**
   * gets the edge value form the text box, if nothing, or an invalid value is entered, then it
   * returns 1.00 by default.
   * 
   ...
   * 
   * @param fieldName - the name of the text box being targeted, should be "start" or "end"
   * @return the id number
   * @throws NumberFormatException - if the text box does not contain parseable integers.
   */
  ...
  public void connect() {
    if (!displaySpanning) {
      try {
        ...
        resetText(false);
        // by default, if vertexes are selected by mouse, create edge with weight 1.00 with the
        // button:
        if (canMakeEdge()) {
  ...
  public void deleteDirectedEdge() {
    if (!displaySpanning) {
      try {
        ...
        // if text unputs do not specify which edge to remove, then remove the edge from the
        // selected
        // vertexes:
        resetText(false);
        if (canRemoveEdge()) {
  ...
  public void deleteUndirectedEdge() {
    if (!displaySpanning) {
      try {
        ...
        resetText(false);
        // if the starting and ending vertexes are not entered through the text line, then remove
        // the
  ...
  private void changeColorBasedOnDegree(int id) {
    ...
    double degreeFraction = ((double) (inDegree + outDegree))
        / ((double) gMain.getEdges().size() + 1.00);
  ...
  /**
   * checks if there exists an edge going either way between the mouse selected vertexes.
   * 
  ...
  private boolean canRemoveEdge() {
    if (canMakeEdge()) {
      if (gMain.getVertex(savedMovers[0].getLabel()).hasOutConnectionTo(savedMovers[1].getLabel())
          || gMain.getVertex(savedMovers[1].getLabel())
              .hasOutConnectionTo(savedMovers[0].getLabel())) {
        return true;
      }
    }
    return false;
   ...
   * 
   * @param validInput - true to turn test green for valid input formatting, false to turn text red
   *                     for invalid input formatting
   */
  ...
  /**
   * performs the specific spanning operation on the main graph from the dropdown menu.
   * 
  ...
  public void resetButtons() {
    ...
    cp5.addTextfield("start").setPosition(10, 0).setSize(40, 40).setAutoClear(false)
        .setText("startID");

    cp5.addTextfield("end").setPosition(50, 0).setSize(40, 40).setAutoClear(false).setText("endID");

    cp5.addTextfield("weight").setPosition(10, 40).setSize(80, 40).setAutoClear(false)
        .setText("edgeWeight");

    cp5.addBang("connect").setPosition(10, 80).setSize(80, 40).setCaptionLabel("Connect")
        .setColorActive(color(0, 255, 0)).getCaptionLabel()
        .align(ControlP5.CENTER, ControlP5.CENTER);

    cp5.addBang("directed").setPosition(10, 120).setSize(80, 40).setColorActive(color(0, 255, 0))
        .setCaptionLabel("Con Start->End").getCaptionLabel()
        .align(ControlP5.CENTER, ControlP5.CENTER);

    cp5.addBang("deleteUndirectedEdge").setPosition(10, 160).setSize(80, 40)
        .setColorActive(color(255, 0, 0)).setCaptionLabel("Disconnect").getCaptionLabel()
        .align(ControlP5.CENTER, ControlP5.CENTER);

    cp5.addBang("deleteDirectedEdge").setPosition(10, 200).setSize(80, 40)
        .setColorActive(color(255, 0, 0)).setCaptionLabel("Dis start->end").getCaptionLabel()
        .align(ControlP5.CENTER, ControlP5.CENTER);
    ...
    cp5.addScrollableList("dropdown").setPosition(10, 300).setSize(80, 40).setBarHeight(20)
        .setItemHeight(20).addItems(buttonList).setCaptionLabel("spanning trees");
  }
}
