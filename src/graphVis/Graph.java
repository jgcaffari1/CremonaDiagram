...
public class Graph<K extends Comparable<K>, V> {
  ...
  /**
   * comparator that sorts each vertex based on its to total weight, useful for spanning trees
   * algorithms.
   */
  ...
  /**
   * adds a new vertex to the graph. If the vertex is already in the graph, then the value is
   * copied.
   * 
   * @param vertex
   */
  ...
  /**
   * gets the total cost of the current graph. Useful for spanning trees algorithms.
   * 
   * @return
   */
   ...
   * @return - false if failure to add edge - meaning the nodes are missing or edge already exists.
   */
  ...
  /**
   * Adds the directed edge from the start vertex to the end vertex. It also assigns a non default
   * weight to the edge.
   * 
   * @param startKey - key for the starting node.
   * @param endKey   - key for the ending node.
   ...
   * @param startKey - key for the starting node.
   * @param endKey   - key for the ending node.
   * @return - false if failure to add both edges - meaning the vertexes are missing or edges
   *         already exist.
   */
   ...
   * 
   * @param startKey - the key where the traversal is starting.
   * @return the list of vertexes from the bft, returns null if the starting vertex is not in the
   *         graph.
   */
  ...
  /**
   * topologically sorts the given graph this algorithm is from lecture26 for cs400
   * 
  ...
  /**
   * resets the state variables for all vertexes for dijkstras and other traversal/ search
   * algorithms
   */
  ...
  private void dfSpanningHelper(K key, ArrayList<Vertex<K, V>> traversal, Graph<K, V> g) {
    ...
    while (keys.hasMoreElements()) {
      ...
      if (!edgesOfCurrentVertex.get(nextKey).getEnd().visited) {
        ...
        // add edge to graph:
        g.directedConnect(currentVertex.getKey(),
            edgesOfCurrentVertex.get(nextKey).getEnd().getKey(),
            edgesOfCurrentVertex.get(nextKey).getWeight());
      }
    }
  }
  ...
  public Graph<K, V> primms(K startKey) {
    ...
    while (!q.isEmpty()) {
      ...
      if (c.predecessor != null) {
        g.directedConnect(c.predecessor.getKey(), c.getKey(),
            c.getInEdge(c.predecessor.getKey()).getWeight());
      }
      // build iterator for the edges:
      edges = c.sortOutEdgesByWeight();
      // evaluate the
   ...
   * 
   * @param startKey - the key to start at
   * @return a graph containing the minimum spanning tree. implementation modified from:
   *         https://ravindraranwala.blogspot.com/2019/12/minimum-spanning-trees-kruskals_8.html
   *         this generates a fully connected minimum spanning tree, however, the directionality of
   *         the connections makes it so that not all edges appear during a breadth first traversal.
   */
  ...
  /**
   * finds the start of a subtree - it should iterate backwards in reverse weight order.
   * 
  ...
  /**
   * initializes the sets for kruskals. All vertexes start as being sets with themselves, i.e they
   * are their own pred.
   */
