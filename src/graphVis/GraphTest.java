...
public class GraphTest {
  ...
  public void Test01_test_basic_connections_and_disconnections() {
    ...
    if (!(g.getVertex(0).getOutEdge(10).equals(edgeForward)
        && g.getVertex(10).getOutEdge(0).equals(edgeReverse))) {
      fail("Edge not formed correctly");
    }

    assertEquals(true, g.disconnect(0, 10));
    if (!(g.getVertex(0).getOutEdge(10) == null
        && g.getVertex(10).getOutEdge(0).equals(edgeReverse))) {
    ...
    if (!(g.getVertex(0).getOutEdge(10).equals(edgeForward)
        && g.getVertex(10).getOutEdge(0) == null)) {
  ...
  /**
   * This algorithm does not seem to work. If you set B to be very large, the algorithm will stop
   * between A and B. need to make it consider the smallest edge weight. It works to some extent,
   * but sometimes it simply doesn't. it is visiting all edges in increasing order , but if b is too
   * high it will stop there.
   */
  @Test
  ...
  public void Test10_kruskals() {
    /**
     * the kruskals implementation creates an undirected spanning tree. this is because it creates a
     * spanning tree that cannot be traversed in a single bfs if directed connections are used.
     */
    Graph<String, Integer> gLocal = new Graph<String, Integer>();
    Graph<String, Integer> gSpan;
    ArrayList<Vertex<String, Integer>> subTreeBFS;
